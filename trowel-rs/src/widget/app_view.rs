use ratatui::{
    buffer::Buffer, layout::{Constraint, Direction, Layout, Rect}, style::{Color, Modifier, Style}, text::{Line, Span}, widgets::{Block, BorderType, Paragraph, StatefulWidget, Widget}
};
...
impl StatefulWidget for AppView {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        let area = if state.show_experimental_warning { a2 } else { area };
...
impl AppView {
    ...
    fn experimental_warning() -> impl Widget {
        let block = Block::bordered().border_type(BorderType::Thick).style(Color::Red);
        let style = Style::default().fg(Color::Red).add_modifier(Modifier::BOLD);
        let text = Line::from(Span::styled("CAUTION: This app is experimental and untested. Do not trust its output!", style));
