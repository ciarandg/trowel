...
def tf_show_json(binary_plan_path):
...
def tf_show_text(binary_plan_path):
    show_result = subprocess.run([TF_BINARY, "show", binary_plan_path], capture_output=True, text=True, check=True)
...
def run(hide_experimental_warning, plan_file, tf_args):
    if plan_file and mimetypes.guess_type(plan_file)[0] == "application/json":
        ...
        json_data = tf_show_json(plan_file)
        text_plan = tf_show_text(plan_file)
    else:
        binary_plan_path = tf_plan(tf_args)
        json_data = tf_show_json(binary_plan_path)
        text_plan = tf_show_text(binary_plan_path)
